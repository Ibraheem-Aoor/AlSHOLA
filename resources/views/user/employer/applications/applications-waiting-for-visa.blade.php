                            ...
                            <tbody>
                                ...
                                @forelse ($applications as $application)
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href="{{ route('employer.application.attachments', $application->id) }}">
                                                {{ $application->attachments_count }}
                                            </a>
                                        </td>
                                        <td>{{ \Carbon\Carbon::parse($application->created_at)->format('Y-M-d')}}</td>
                                        <td>{{ $application->visa->arrival_date ?? ''}}</td>
                                        <td>
                                            <a href="#exampleModal_5" class="btn btn-outline-success"
                                                data-id="{{ $application->id }}" data-toggle="modal"><i
                                                    class="fa fa-upload"></i> VISA FILE</a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">UPLOAD VISA DOCUMENT:</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form action="{{ route('employer.application.visa.upload') }}" method="POST"
                                enctype="multipart/form-data">
                                @csrf
                                <div class="form-group col-sm-6">
                                    <label for="" style="font-weight: 600">VISA DOCUMENT:</label>
                                    &nbsp; &nbsp; <input type="file" required name="file" class="form-control">
                                    &nbsp; &nbsp; <input type="text" id="id" name="id" hidden>
                                </div>
                                <div class="form-group col-sm-6">
                                    <label for="" style="font-weight: 600">VISA NUMBER:</label>
                                    <input type="text" name="number" class="form-control" required>
                                </div>
                                <div class="form-group col-sm-6 mb-2">
                                    <label for="" style="font-weight: 600">VISA EXPIRE DATE:</label>
